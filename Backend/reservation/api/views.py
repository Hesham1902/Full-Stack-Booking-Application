...
def create_reservation(request):
    ...
    if serializer.is_valid():
        ...
        if len(set(reserved_dates)) != len(reserved_dates):
            return Response({"detail": "Duplicate dates are not allowed."},
    ...
    # Check if reserved dates fall on working days of the studio
        for date_str in reserved_dates:
            ...
            if reserved_date.strftime('%A') not in studio.working_days.split(','):
                return Response({"detail": f"[{date_str} - {reserved_date.strftime('%A')}] is not a working day for the studio."},
                                status=status.HTTP_400_BAD_REQUEST)
...
def delete_reservation(request, reservation_id):
    ...
    if current_time >= (reservation.created_at + timedelta(minutes=15)):
        return Response({"error": "Timeover you can't cancel the reservation now!"}, status=status.HTTP_400_BAD_REQUEST)
...
# class StudioReservationsListView(generics.ListAPIView):
#     serializer_class = ReservationSerializer
#     permission_classes = [IsAuthenticated, permissions.IsStudioOwnerType]

#     def get_queryset(self):
#         # Get the studios owned by the logged-in studio owner
#         owned_studios = self.request.user.studios.all()

#         # Get all reservations associated with the owned studios
